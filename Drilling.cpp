...
int main()
{
	...
	DrillingRecordArray dra = DrillingRecordArray();
	long resultL = resultLine.size() - 1;
	unsigned int countSize = 0;

	for (long i = resultL; i > 0; i = i - 18) 
	{
		DrillingRecord* tempDr = new DrillingRecord;
		tempDr->addString(resultLine[i - 17]);
		tempDr->addString(resultLine[(i - 16)]);
		for (int index = 0; index < 16; ++index)
		{
			double temp = stof(resultLine[(i - 15 + index)].c_str());
			tempDr->addNum(temp);
		}
	...
	for (unsigned i = 0; i < countSize; ++i) 
	{
		cout << dra.get(i) << endl;
	}
	return 0;
